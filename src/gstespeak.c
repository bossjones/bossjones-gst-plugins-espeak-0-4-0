...
static GstFlowReturn gst_espeak_create(GstBaseSrc *, guint64, guint,
                                       GstBuffer **);
...
static void gst_espeak_class_init(GstEspeakClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("text", "Text", "Text to pronounce", NULL,
                          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property(
      gobject_class, PROP_PITCH,
      g_param_spec_int("pitch", "Pitch adjustment", "Pitch adjustment", -100,
                       100, 0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property(
      gobject_class, PROP_RATE,
      g_param_spec_int("rate", "Speed in words per minute",
                       "Speed in words per minute", -100, 100, 0,
                       G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property(
      gobject_class, PROP_VOICE,
      g_param_spec_string("voice", "Current voice", "Current voice",
                          ESPEAK_DEFAULT_VOICE,
                          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property(
      gobject_class, PROP_GAP,
      g_param_spec_uint("gap", "Gap", "Word gap", 0, G_MAXINT, 0,
                        G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property(
      gobject_class, PROP_TRACK,
      g_param_spec_uint("track", "Track", "Track espeak events", 0, G_MAXINT,
                        ESPEAK_TRACK_NONE,
                        G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property(
      gobject_class, PROP_VOICES,
      g_param_spec_boxed("voices", "List of voices", "List of voices",
                         G_TYPE_VALUE_ARRAY,
  ...
  gst_element_class_set_metadata(
      element_class, "eSpeak as a sound source", "Source/Audio",
...
static void gst_espeak_init(GstEspeak *self) {
  ...
  self->caps = gst_caps_new_simple("audio/x-raw", "format", G_TYPE_STRING,
                                   gst_audio_format_to_string(format), "layout",
                                   G_TYPE_STRING, "interleaved", "rate",
                                   G_TYPE_INT, espeak_get_sample_rate(),
...
static GstFlowReturn gst_espeak_create(GstBaseSrc *self_, guint64 offset,
...
static gboolean gst_espeak_is_seekable(GstBaseSrc *src) { return FALSE; }
...
static GstURIType gst_espeak_uri_get_type(GType type) { return GST_URI_SRC; }
...
static gboolean gst_espeak_uri_set_uri(GstURIHandler *handler, const gchar *uri,
                                       GError **error) {
  gchar *protocol, *text;
  gboolean ret;
...
static gboolean espeak_init(GstPlugin *espeak) {
  ...
  return gst_element_register(espeak, "espeak", GST_RANK_NONE, GST_TYPE_ESPEAK);
}

GST_PLUGIN_DEFINE(GST_VERSION_MAJOR, GST_VERSION_MINOR, espeak,
                  "Uses eSpeak library as a sound source for GStreamer",
                  espeak_init, PACKAGE_VERSION, "LGPL", PACKAGE_NAME,
